...
A.hud=()=>{let loc=A.scene==='inn'?'Karaova Hanı':'Karaova',skills=Object.values(P.sk).filter(Boolean).length,left={x:12,y:10,w:238,h:70},right={x:A.vw-262,y:10,w:250,h:70};
 ...
 A.drawHudPanel(right.x,right.y,right.w,right.h,()=>{let x=right.x+14;g.textAlign='left';g.textBaseline='alphabetic';g.font='700 10px sans-serif';g.fillStyle='#d8c39d';g.fillText(`TAHIL ${P.bag.grain}  •  BALIK ${P.bag.fish||0}`,x,31);g.font='10px sans-serif';g.fillStyle='#f0dfbd';g.fillText(`BECERİ ${skills}/4  •  ÜN ${P.fame||0}`,x,48);let extra=P.room?'ODA ANAHTARI ✓':`VAREN ${P.rel.Varen||0}/${A.M.Varen?A.M.Varen[2]:4}`;g.fillStyle='#b9aa8d';g.fillText(extra,x,65)});
