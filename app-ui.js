...
A.hud=()=>{let loc=A.scene==='inn'?'Karaova Hanı':A.scene==='home'?'Kendi Evin':'Karaova',skills=Object.values(P.sk).filter(Boolean).length,cnt=id=>A.inv?A.inv.count(id):(P.bag[id]||0),left={x:12,y:10,w:238,h:70},right={x:A.vw-262,y:10,w:250,h:82};
 ...
 A.drawHudPanel(right.x,right.y,right.w,right.h,()=>{let x=right.x+14;g.textAlign='left';g.textBaseline='alphabetic';g.font='700 10px sans-serif';g.fillStyle='#d8c39d';g.fillText(`TAHIL ${cnt('grain')}  •  BALIK ${cnt('fish')}`,x,27);g.font='10px sans-serif';g.fillStyle='#d7cbb8';g.fillText(`DEMİR ${cnt('iron')}  •  ALET ${cnt('tools')}`,x,43);g.fillStyle='#f0dfbd';g.fillText(`BECERİ ${skills}/4  •  ÜN ${P.fame||0}`,x,59);let extra=A.shop&&A.shop.owned?`KASA ${A.shop.cash} • ${A.shop.worker?(A.shop.workerName||'ÇALIŞAN'):'SEN'}`:A.home&&A.home.owned?'EV SAHİBİ ✓':P.room?'ODA ANAHTARI ✓':`VAREN ${P.rel.Varen||0}/${A.M.Varen?A.M.Varen[2]:4}`;g.fillStyle='#b9aa8d';g.fillText(extra,x,75)});
