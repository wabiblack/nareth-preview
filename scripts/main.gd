...
func _ready() -> void:
	...
	if atlas_texture == null:
		_set_status("Atlas okunamadi • Output panelini kontrol et")
	...
	_build_karaova()
	_build_player()
	_set_status("Karaova • native Godot prototipi")
...
func _load_atlas_from_web_chunks() -> ImageTexture:
	...
	for i: int in range(6):
		...
		if i == 0:
			...
			var first_marker: int = txt.find(first_token)
			if first_marker >= 0:
				start = first_marker + first_token.length()

		if start < 0:
			var append_token: String = "+='"
			var append_marker: int = txt.find(append_token)
			if append_marker >= 0:
				start = append_marker + append_token.length()

		if start < 0:
			var legacy_token: String = "+'"
			var legacy_marker: int = txt.find(legacy_token)
			if legacy_marker >= 0:
				start = legacy_marker + legacy_token.length()

		var finish: int = txt.rfind("'")
		if start < 0 or finish <= start:
			push_error("Atlas parcasi okunamadi: %s, start=%d finish=%d" % [path, start, finish])
			return null

		b64 += txt.substr(start, finish - start)

	if b64.is_empty():
		push_error("Atlas base64 verisi bos")
		return null
	...
	if err != OK:
		push_error("PNG decode hatasi: %s • bytes=%d" % [error_string(err), raw.size()])
		return null
...
func _build_boot_background() -> void:
	...
	road.width = 100.0
	road.default_color = Color8(126, 94, 59)
	road.antialiased = false
...
func _build_background() -> void:
	...
	var road_base: Line2D = Line2D.new()
